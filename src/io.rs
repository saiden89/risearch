...
pub fn read_fasta_sequences(filename: impl AsRef<Path>) -> Result<FastaRecords> {
    ...
    let md = std::fs::metadata(filename_ref)
        .with_context(|| format!("Failed to access FASTA/FASTQ file: {}", filename_ref.display()))?;
    if md.is_file() && md.len() == 0 {
        return Ok(Vec::new());
    }

    let mut reader = parse_fastx_file(filename_ref)
        .with_context(|| format!("Failed to open FASTA/FASTQ file: {}", filename_ref.display()))?;
    let mut records = Vec::new();

    while let Some(record) = reader.next() {
        let rec = record
            .with_context(|| format!("Failed to parse FASTA/FASTQ record from {}", filename_ref.display()))?;
