...
fn hit_query_bases<'a>(hit: &SearchHit, q_seq: &'a [Base]) -> &'a [Base] {
    ...
    if end_excl < start { &q_seq[0..0] } else { &q_seq[start..end_excl] }
}
...
fn hit_target_bases<'a>(hit: &SearchHit, t_fwd: &'a [Base], t_rc: &'a [Base]) -> &'a [Base] {
    match hit.strand {
        crate::types::Strand::Forward => {
            ...
            if end_excl < start { &t_fwd[0..0] } else { &t_fwd[start..end_excl] }
        }
        crate::types::Strand::Reverse => {
            ...
            if end_excl < start { &t_rc[0..0] } else { &t_rc[start..end_excl] }
        }
    }
}
...
/// Returns `(flank_5, flank_5_rev, flank_3, flank_3_rev)` slices in oriented-strand space.
///
/// In binding-site output the C `-p3` naming is swapped vs. our internal names:
/// the output `flank5` column is our `flank_3` complemented, and vice-versa.
    ...
    for (idx, field) in spec.fields.iter().enumerate() {
        ...
        match field {
            ...
            FieldKind::TargetSeq => {
                if let Some(align) = alignment {
                    let t_bases = hit_target_bases(hit, t_fwd, t_rc);
                    push_alignment_target_seq_bindingsite(out, align, t_bases);
                }
            }
            // C `-p3` semantics: output `flank5` column = our `flank_3` complemented, and vice-versa.
            FieldKind::Flank5 => {
