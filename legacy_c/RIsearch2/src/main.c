...
int all_vs_all = 1;
int no_compress = 0; // When 1, write to stdout uncompressed
...
void usage(const char *cmd) {
  ...
  fprintf(stderr,
          ...
          "===============================\n",
          version);
  fprintf(stderr, "================ Energy based RNA-RNA interaction "
                  "predictions ================\n\n");
  ...
  fprintf(stderr, "                 create suffix array for target sequence(s) "
                  "together with\n");
  fprintf(stderr, "                 their reverse complements, FASTA format, "
                  "use '-' for stdin\n");
  ...
  fprintf(stderr, "                 pregenerated suffix array file for target "
                  "sequence(s)\n");
  fprintf(stderr, "  -s n:m/l,   --seed=n:m/l\n");
  fprintf(stderr, "                 set seed length (-s l = length only; -s "
                  "n:m = full interval;\n");
  fprintf(stderr,
          "                 -s n:m/l = length in interval; default -s 6)\n");
  fprintf(stderr, "  -l <int>,   --extension=L \n");
  fprintf(stderr, "                 max extension length(L) on the seed (do DP "
                  "for max this length\n");
  fprintf(stderr,
          "                 up- and downstream of seed) (default L=20)\n");
  fprintf(stderr, "  -e <float>, --energy=dG\n");
  fprintf(stderr, "                 set deltaG energy threshold (in kcal/mol) "
  ...
  fprintf(stderr, "                 report only predictions with a matching 3' "
                  "PAM forward complement\n");
  fprintf(stderr, "                 for e.g. cas9 this corresponds to "
                  "PC=^(.cc|.uc|.cu) matching NGG/NAG/NGA 3' PAMs\n");
  fprintf(stderr, "                 report mode 3/4 required\n");
  fprintf(stderr, "  -5 <reg>,   --five_prime_match=PC\n");
  fprintf(stderr, "                 report only predictions with a matching 5' "
                  "PAM reverse complement\n");
  fprintf(stderr, "                 for e.g. cas12a this corresponds to "
                  "PC=^([^a]aaa) matching TTTV 5'PAMs\n");
  fprintf(stderr, "                 report mode 3/4 required\n");
  fprintf(stderr, "  -1          --one_vs_one only print results where query "
                  "name matches target name\n");
  ...
  fprintf(stderr, "                 report predictions in a simple format "
                  "together with CIGAR-like \n");
  ...
  fprintf(stderr, "                 (required for post-processing of CRISPR "
                  "off-target predictions)\n");
  fprintf(stderr, "  -p4,        --report_alignment=3   \n");
  fprintf(stderr, "                 report predictions in a very short format "
                  "target, start and strand + energy\n");
  fprintf(stderr, "  --noGUseed     consider G-U wobble pairs as mismatch "
                  "within the seed\n");
  fprintf(stderr, "                 (only for locating seeds, energy model is "
                  "not affected)\n");
  fprintf(stderr, "  --verbose      verbose output\n");
  fprintf(stderr, "  -m c        --mismatch=c       Mismatched seeds with max "
                  "num of mismatches (c)\n");
  fprintf(stderr, "  -m c:p      --mismatch=c:p      min consecutive matches "
                  "at seed start/end (p)\n");
  fprintf(stderr, "  -m c:ps:pe  --mismatch=c:ps:pe  or (ps) matches at the "
                  "start & (pe) at the end\n");
  fprintf(stderr, "                 These seeds will not overlap with perfect "
                  "complementary seeds\n");
  fprintf(stderr, "                  (default -m 0:0 means no mismatch         "
                  "                 )\n");
  fprintf(stderr, "                  (when c>0, please set p>0 to avoid "
                  "overlaps, p=c at default)\n");
  fprintf(stderr, "  -x <float>, --seed_energy=F\n");
  fprintf(stderr, "                 set energy per length threshold that "
                  "filters seeds (default=0)\n");
  fprintf(stderr, "  -z mat,     --matrix=mat\n");
  fprintf(stderr, "                 su95 for RNA-DNA duplexes\n");
  fprintf(
      stderr,
      "                 su95c2 for RNA-DNA duplexes modified for CRISPRoff2\n");
  fprintf(stderr, "                 su95wk11 for RNA-DNA duplexes with "
                  "mismatches open/close as loop size 2\n");
  fprintf(stderr, "                 su95_noGU for RNA-DNA duplexes (G-T and "
                  "U-G are mismatches)\n");
  fprintf(
      stderr,
      "                 sl04_noGU for DNA-DNA duplexes (G-T is a mismatch)\n");
  fprintf(stderr, "                 (last two models automatically activates "
                  "the --noGUseed option)\n");
  fprintf(stderr, "  -y mat2,     --matrix2=mat2\n");
  fprintf(
      stderr,
      "                 Only needed if you design your own energy matrices\n");
  fprintf(stderr, "  -b band,     --band=band\n");
  fprintf(stderr, "                 Integer, size of the bands that limit the "
                  "search for bulged matches.\n");
  fprintf(stderr, "                 The minimum size is 1; please use the seed "
                  "option to avoid any bulge.\n");
  fprintf(stderr, "  -w arr,     --weights=arr\n");
  fprintf(stderr, "                 CRISPR_gRNApPAM to weight gRNA-target "
                  "interactions by CRISPR/Cas9 impact.\n");
  fprintf(stderr, "  -K T1[,T2,T3],     --temperature=T0[,T1,T2]\n");
  fprintf(stderr, "                 Temperatures in Kelvin for scaling of "
                  "energies. T0 is the temperature to\n");
  fprintf(stderr, "                 be scaled to. T1 and T2 are only needed if "
                  "you design your own energy parameters.\n");
  fprintf(stderr,
          "  -M PATH,     Path to directory holding the energy matrices\n");
  fprintf(stderr, "\n");
  exit(1);
}
...
void options(int argc, char *argv[]) {
  ...
  while ((c = getopt_long(argc, argv,
                          "13:5:b:c:d:e:hi:K:l:m:M:no:p::q:s:t:Uvw:x:y:z:",
    ...
    case 'e':
      ...
      if (*ptr) {
        // Did not read until \0, came across something that could not be
        // converted to float
    ...
    case 'l':
      ...
      break;
    case 'm':
      /* "-m c:s:e" c:allowed number of mismatches. s/e is the number of
       * consecutive matches required at the beginning/end of the seed.*/
      ...
      if (!optarg) {
        ...
        debug("opt: seed_mismatch=(%d,%d,%d)\n", seed_mismatch[0],
              seed_mismatch[1], seed_mismatch[2]);
        break;
      }
      optarg++;
    ...
    case '?':
      fprintf(stderr,
              "Parameter -%c ignored: unknown option or missing argument.\n",
              optopt);
      err_flag = 1;
      break;
    }
  ...
  if (err_flag) {
    fprintf(stderr, "\nThere was a problem with the parameters passed, please "
...
int getMat(const char *matname, int *bA_nu) {
  ...
  if (!strcmp(matname, "t04")) {
    ...
    fprintf(stderr,
            "Undefined matrix, -m needs to be set to either t99 or t04\n");
    exit(1);
  }
  ...
  for (i = 0; i < 1296; i++) {
    *(bA_nu + i) =
        *(bA_bas + i) - extPen * *(bA_ext + i); /* bA_nu[i] =  ... also works */
                                                /*
                                                //print dsm
 ...
 *  @param n_read The address to hold the number of sequences read (#entries in
 * returned)
 *
...
query_t *read_queries(fasta_t *ffp, int max_to_read, int *n_read) {
  ...
  while (fasta_read(ffp, &queries[n_seq].seq, &queries[n_seq].name,
                    &queries[n_seq].length)) {
    ...
    sa_create_partial_reverse(query->seq, query->seed[0] - 1,
                              query->seed[1] - 1, query->seed[2], &query->qsa,
                              &query->length);
    n_seq += 1;
    *n_read = n_seq;
    ...
    for (i = 0; i < d->n_queries; i++) {
      ...
      if (i % nthreads == tid) {
...

        {
          ...
          // should probably get a basename as parameter and combine to:
          ...
          // query->length);
            ...
            sa_parallel_match_neg(query->qsa, 0, query->length, d->sa, 0, d->n,
                                  0, query->seed[2], 0, 0, &query->intervals);
...
int main(int argc, char *argv[]) {
  ...
  if (fasta_raw && output) {
    ...
    // n is twice the number of nucleotides in input, because of reverse
    // complement k is twice the number of sequences in input, because of
    // reverse complement
    debug("N=%li, K=%li\n", n, num_idxs);
