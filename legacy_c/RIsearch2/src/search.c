...
// saidx64_t *seed_lengths; //array storing how many maximal seeds of each size
// are found; counts are incremented in the extend_seed function
...
//!!! not used !!!
saidx64_t binary_search(saidx64_t A[], saidx64_t key, saidx64_t imin,
                        saidx64_t imax) {
 ...
 * Example: Already found the interval of all suffixes starting with agg, now
 * search aggC at offset 3.
 *
 * @param[in] sa The address to the suffix array (either query or target)
 * @param[in] start The first position (in SA) of the interval to examine/step
 * into
 ...
 */
saidx64_t sa_search_left(const saidx64_t *sa, saidx64_t start, saidx64_t end,
                         saidx64_t offset, saint_t c) {
  saidx64_t half;

  half = (end - start) >> 1;
  while (start < end) {
    // fprintf(stderr, "start: %lu half: %lu offset: %lu XSA(sa[start+half]):
    // %lu\n", start, half, offset, XSA(sa[start + half]));
    saidx64_t xsa_i = XSA(sa[start + half]) + offset;
    // fprintf(stderr, "xsa_i = XSA(sa[start + half]) + offset = %lu ;
  ...
  // fprintf(stderr, "found pos=%lu; XSA(sa[start])+offset=%lu+%lu = %lu ;
  // XSTRM(sa[XSA(sa[start]) + offset] = %c \n", start, XSA(sa[start]), offset,
  // XSA(sa[start])+offset, XSTRM(sa[XSA(sa[start])+offset]));
  return start;
}
 ...
 */
saidx64_t sa_search_interval(const saidx64_t *sa, saidx64_t start,
                             saidx64_t end, saidx64_t offset,
                             saidx64_t interval[6]) {
  int i;
  for (i = 0; i < 5; i++) {
 ...
 */
int DP_left(
    int *M,  //!<[out] DP matrix for match state
...
#define TI(q, t) ((q) * (lt) + (t))
  ...
  *best_j = 0;
  ...
  for (i = 3; i < lq; ++i) {
    for (j = 3; j < lt; ++j) {
      M[TI(i, j)] =
          max3(M[TI(i - 1, j - 1)] + (*S)[Q(i)][Q(i - 1)][T(j)][T(j - 1)],
               Bq[TI(i - 1, j - 1)] + (*S)[Q(i)][Q(i - 1)][T(j)][GAP],
               Bt[TI(i - 1, j - 1)] + (*S)[Q(i)][GAP][T(j)][T(j - 1)]);

      Bq[TI(i, j)] = max(M[TI(i - 1, j)] + (*S)[Q(i)][Q(i - 1)][GAP][T(j)],
                         Bq[TI(i - 1, j)] + (*S)[Q(i)][Q(i - 1)][GAP][GAP]);
      Bt[TI(i, j)] = max(M[TI(i, j - 1)] + (*S)[GAP][Q(i)][T(j)][T(j - 1)],
...
/**
 * @brief Dynamic programming extension "right" of the seed (downstream on
 * query)
 *
...
#define TI(q, t) ((q) * (lt) + (t))
  ...
  *best_j = 0;
  ...
  for (j = 2; j < lt; ++j) {
    ...
    // t_start, lt); fprintf(stderr, "TI(0,j): %d\n", TI(0,j));
  ...
  for (j = 3; j < lt; ++j) {
    // i=1: M already set; Bq is NA; Bt as in main recursion
    Bt[TI(1, j)] = max(M[TI(1, j - 1)] + (*S)[Q(1)][GAP][T(j - 1)][T(j)],
                       Bt[TI(1, j - 1)] + (*S)[GAP][GAP][T(j - 1)][T(j)]);
    // 3rd row (i=2)
    // Bq is not a possible source for M and Bq
    M[TI(2, j)] = max(M[TI(1, j - 1)] + (*S)[Q(1)][Q(2)][T(j - 1)][T(j)],
                      Bt[TI(1, j - 1)] + (*S)[GAP][Q(2)][T(j - 1)][T(j)]);
    Bq[TI(2, j)] = M[TI(1, j)] + (*S)[Q(1)][Q(2)][T(j)][GAP];
    // Bt as in main recursion
    Bt[TI(2, j)] = max(M[TI(2, j - 1)] + (*S)[Q(2)][GAP][T(j - 1)][T(j)],
  ...
  for (i = 3; i < lq; ++i) {
    for (j = 3; j < lt; ++j) {
      M[TI(i, j)] =
          max3(M[TI(i - 1, j - 1)] + (*S)[Q(i - 1)][Q(i)][T(j - 1)][T(j)],
               Bq[TI(i - 1, j - 1)] + (*S)[Q(i - 1)][Q(i)][GAP][T(j)],
               Bt[TI(i - 1, j - 1)] + (*S)[GAP][Q(i)][T(j - 1)][T(j)]);

      Bq[TI(i, j)] = max(M[TI(i - 1, j)] + (*S)[Q(i - 1)][Q(i)][T(j)][GAP],
                         Bq[TI(i - 1, j)] + (*S)[Q(i - 1)][Q(i)][GAP][GAP]);
      Bt[TI(i, j)] = max(M[TI(i, j - 1)] + (*S)[Q(i)][GAP][T(j - 1)][T(j)],
...
void print_alignment(
    const saidx64_t *sa,  //!<[in] Suffix array of the target
    const saidx64_t *qsa, //!<[in] Suffix array of the query
    aln_result_t *result, //!<[in,out] result struct: gets positions (seed and
                          //!< best dp), sets query/aln/target_str and new_ pos
    ...
    if (state == MA) {
      ...
      if (Q(i) == T(j) && Q(i) != 5) // same nt, not 'N'
        strcat(temp_aln_str, "|");
          ...
          /*(noGUseed==0) & this would be necessary if you don't want to show
             G-Us with ":" */
      ...
      if (M[TI(i, j)] ==
          M[TI(i - 1, j - 1)] + (*S)[Q(i)][Q(i - 1)][Tc(j)][Tc(j - 1)]) {
        state = MA;
      } else if (M[TI(i, j)] ==
                 Bq[TI(i - 1, j - 1)] + (*S)[Q(i)][Q(i - 1)][Tc(j)][GAP]) {
        state = BQ;
      } else if (M[TI(i, j)] ==
                 Bt[TI(i - 1, j - 1)] + (*S)[Q(i)][GAP][Tc(j)][Tc(j - 1)]) {
        state = BT;
      }
      i--;
      j--;
      ...
      if (Bq[TI(i, j)] == M[TI(i - 1, j)] + (*S)[Q(i)][Q(i - 1)][GAP][Tc(j)]) {
        state = MA;
        i--;
      } else if (Bq[TI(i, j)] ==
                 Bq[TI(i - 1, j)] + (*S)[Q(i)][Q(i - 1)][GAP][GAP]) {
      ...
      if (Bt[TI(i, j)] == M[TI(i, j - 1)] + (*S)[GAP][Q(i)][Tc(j)][Tc(j - 1)]) {
        state = MA;
        j--;
      } else if (Bt[TI(i, j)] ==
...
#define TI(q, t) ((q) * (rem_target_len_right) + (t))
  ...
  while (i > 0 || j > 0) {
    if (state == MA) {
      ...
      if (M[TI(i, j)] ==
          M[TI(i - 1, j - 1)] + (*S)[Q(i - 1)][Q(i)][Tc(j - 1)][Tc(j)]) {
        state = MA;
      } else if (M[TI(i, j)] ==
                 Bq[TI(i - 1, j - 1)] + (*S)[Q(i - 1)][Q(i)][GAP][Tc(j)]) {
        state = BQ;

      } else if (M[TI(i, j)] ==
                 Bt[TI(i - 1, j - 1)] + (*S)[GAP][Q(i)][Tc(j - 1)][Tc(j)]) {
        state = BT;
      }
      i--;
      j--;
...
void print_ali2(
    ...
    const saidx64_t *qsa, //!<[in] Suffix array of the query
    aln_result_t *result, //!<[in,out] result struct: gets positions (seed and
                          //!< best dp), sets ia_str and new_ pos
    saidx64_t query_len,  //!<[in] Length of the query
    int seed_len,         //!<[in] Length of the seed
    int *M_left,          //!<[in] DP matrix for left extension, match state
    int *Bq_left,  //!<[in] DP matrix for left extension, query bulged state
...
void print_result(char *qname, const saidx64_t *sa, const saidx64_t *qsa,
                  aln_result_t *result) {
  saidx64_t lpos, rpos;
  char strand;
  ...
  while (imax >= imin) {
    ...
    if (sum_l[imid] <= result->new_left && sum_l[imid + 1] > result->new_left) {
      idx = imid;
      break;
    }
  ...
  /*
     ...
     result->energy);
   */
  sprintf(result_str, "%s\t%lu\t%lu\t%s\t%lu\t%lu\t%c\t%.2f", qname,
          result->q_new_left + 1, result->q_new_right + 1, name[idx], lpos,
          rpos, strand, result->energy);
}
 ...
 * Tests whether seed match is maximal, by checking whether flanked to the left
 * or right by a valid base pair, Only performs DP extension if seed is maximal,
 * because otherwise, it is contained into a maximal seed. Finds boundaries for
 * DP, Calculates scores for left/right extension (calls) Transform to energy
 * and print results (call)
 *
 * @return Void.
 */
void extend_seed(
    saidx64_t j, //!< Index in target suffix array (where seed match was found)
    ...
    saidx64_t
        query_len, //!< Length of the query, target length is in sum_l table
    int seed_len,  //!< Length of the seed
    saidx64_t best_spos, //!< Start pos. of the most energy favourable subseed
                         //!< within any seed
    ...
    int *Bt_right)       //!< DP matrix for right extension, target bulged state
{
  ...
  saidx64_t best_left_i = 0, best_left_j = 0, best_right_i = 0,
            best_right_j = 0;

  result->found = 0;

  // get 0-based suffix numbers for query and target, and get seq id of the
  // target region
...
#define T(ix) (comp[XRIS(sa[t_start + (ix)])])
  ...
  // in that case, left maximality is important only if seed starts in mid
  // positions. seed_flag determines if there is a positional constraint
  if (noGUseed) {
    if (seed_flag) {
      if (q_start + 1 < seed[0] || q_start + q_seed_len > seed[1]) {
        if (print_debug) {
          fprintf(
              stderr,
              "extension stopped due to start/end position for q_seed:%lu-%lu, "
              " rem_query_len_left: %d, rem_query_len_right: %d, "
              "rem_target_len_left: %d, rem_target_len_right: %d \n",
              q_start + 1, q_start + q_seed_len, rem_query_len_left,
              rem_query_len_right, rem_target_len_left, rem_target_len_right);
        }
        // extension stops due to start or end position
        return;
      }

      if ((rem_query_len_left - (seed[0] - 1) > 0 && rem_target_len_left > 0 &&
           pair_noGU[Q(-1)][T(-1)]) ||
        ...
        if (print_debug) {
          fprintf(stderr,
                  "extension stopped due to maximality for q_seed:%lu-%lu,  "
                  "rem_query_len_left: %d, rem_query_len_right: %d, "
                  "rem_target_len_left: %d, rem_target_len_right: %d \n",
                  q_start + 1, q_start + q_seed_len, rem_query_len_left,
                  rem_query_len_right, rem_target_len_left,
                  rem_target_len_right);
        }
        // extension stops due to maximality within the seed
        return;
      }
    } else if ((rem_query_len_left > 0 && rem_target_len_left > 0 &&
                pair_noGU[Q(-1)][T(-1)]) ||
               (rem_query_len_right > 0 && rem_target_len_right > 0 &&
                pair_noGU[Q(q_seed_len)][T(t_seed_len)])) {
      if (print_debug) {
        fprintf(stderr,
                "stop extension whether maximality or str/end position! %lu "
                "%lu %d %d\n",
    ...
    if (seed_flag) {
      ...
      if ((rem_query_len_left - (seed[0] - 1) > 0 && rem_target_len_left > 0 &&
           pair[Q(-1)][T(-1)]) ||
          (rem_query_len_right - (query_len - seed[1]) > 0 &&
           rem_target_len_right > 0 && pair[Q(q_seed_len)][T(t_seed_len)])) {
        if (print_debug) {
          fprintf(stderr,
                  "extension stopped due to maximality for q_seed:%lu-%lu,  "
                  "rem_query_len_left: %d, rem_query_len_right: %d, "
...
#undef T
  ...
  // Remaining sequence length definition has to change here since we are going
  // to decide DP boundaries
  /* find boundaries for DP (remaining seq left/right of the seed)
   * all rem_{query,target}_len_{left,right} are + 1
   * i.e. nnnNNNNNnnnnn (with N for seed location) yields rem..left=4,
   * rem..right=6 find value for query by actual boundaries (limited by
   * max_ext_len) for target its also limited by max_ext_len, possibly corrected
   * by sequence boundaries
   */
  ...
  total_score +=
      DP_left(M_left, Bq_left, Bt_left, rem_query_len_left, rem_target_len_left,
              qsa, sa, q_start, t_start, &best_left_i, &best_left_j);
  ...
  int nt_count = best_left_j + best_left_i + best_right_i + best_right_j +
  ...
  if (print_debug) {
    fprintf(stderr,
            ...
            rem_target_len_right);
    fprintf(stderr,
            "nt_count is %d, using blj %li, bli %li, bri %li, brj %li, "
            "q_seedlen: %d, t_seedlen: %d \n",
  ...
  /*Energy correction, BE AWARE of energy correction, if one energy matrix is
   * accepted as input there will be some option here */
  result->energy = (result->score - 559.0f) / -100.0f;

  // ENERGY THRESHOLD for the interaction
  if (result->energy > min_energy) {
    fprintf(stdout,
            "C_DEBUG: extend_seed: seed_energy=%d, l_score=%d, r_score=%d, "
            "raw_total=%d, final=%.2f\n",
  ...
  {
    if (show_alignment == 1) {
      print_alignment(sa, qsa, result, query_len, seed_len, M_left, Bq_left,
                      Bt_left, M_right, Bq_right, Bt_right);
    } else if (show_alignment == 2) {
      print_ali2(sa, qsa, result, query_len, seed_len, M_left, Bq_left, Bt_left,
                 M_right, Bq_right, Bt_right);
    } else if (show_alignment == 3) {
      print_alignment(sa, qsa, result, query_len, seed_len, M_left, Bq_left,
                      Bt_left, M_right, Bq_right, Bt_right);
      print_ali2(sa, qsa, result, query_len, seed_len, M_left, Bq_left, Bt_left,
...
void sa_evaluate_interval(sa_interval_list_t *results, query_t *query,
  ...
  if (max_ext_len > 0) {
    ...
    str_size =
        4 * max_ext_len +
        query_len; // should be enough to allow for x gaps on x nts extension
  } else {
    ...
    if (seed_threshold_flag) {
      ...
      for (j = seed_len - 1; j >= (min_seed_length - 1); j--) {
        ...
        for (k = 0; k <= (j - (min_seed_length - 1)); k++) {
          ...
          temp_subseed_score -= (*S)[Q(k)][Q(k + 1)][T(k)][T(k + 1)];
        }
        temp_subseed_score = hold_score - (*S)[Q(j - 1)][Q(j)][T(j - 1)][T(j)];
      }
...
#undef T
    ...
    for (j = sl; j < sr; j++) {
      ...
      if (xsa + seed_len > sum_l[idx + 1]) {
        ...
        // Since all valid seeds, whether maximal or not, are feeded into this
        // stage Ignoring seeds that extends into next sequence is not a problem
        continue;
      }
      ...
      for (k = ql; k < qr; k++) {
        result->j = j;
        result->k = k;
        // debug("\n seed len = %d, query len = %d, seed score = %d , t_start =
        ...
        extend_seed(j, k, sa, qsa, result, seed_score, query->length, seed_len,
                    best_subseed_spos, best_subseed_len, best_subseed_score,
                    query->name, query->seed, M_left, Bq_left, Bt_left, M_right,
                    Bq_right, Bt_right);

        // #pragma omp critical
        // NOT critical as now all write to own file! reduces overhead that is
        // spent waiting?
        {
          if (result->found)
 ...
 * Collects quadrupels (sl,sr,ql,qr) that define all seed match intervals, when
 * the minimum seed length is reached.
 *
 * @return Void.
 */

void sa_parallel_match_neg(
    const saidx64_t *qsa, //!< Suffix array of the query
    saidx64_t ql, //!< The first position (in query SA) of the interval to
                  //!< examine/step into
    saidx64_t qr, //!< The last position (exclusive) of that interval
    const saidx64_t *sa, //!< Suffix array of the target
    saidx64_t sl,     //!< The first position (in target SA) of the interval to
                      //!< examine/step into
    ...
    int last_match_count, //!< Current number of matches after the last
                          //!< mismatch.
    ...
    sa_interval_list_t **results) //!< List of intervals with seed matches
{
  ...
  if (qint[1] - qint[0]) {
    if (sint[1] - sint[0])
      sa_parallel_match_neg(qsa, qint[0], qint[1], sa, sint[0], sint[1], offset,
                            depth, last_match_count + 1, mismatch_count,
                            results);
  }
  /* c */
  if (qint[2] - qint[1]) {
    if (sint[2] - sint[1])
      sa_parallel_match_neg(qsa, qint[1], qint[2], sa, sint[1], sint[2], offset,
                            depth, last_match_count + 1, mismatch_count,
                            results);
  }
  /*  g */
  if (qint[3] - qint[2]) {
    if (sint[3] - sint[2])
      sa_parallel_match_neg(qsa, qint[2], qint[3], sa, sint[2], sint[3], offset,
                            depth, last_match_count + 1, mismatch_count,
                            results);
    /* wobble pair */
  ...
  if (qint[5] - qint[4]) {
    if (sint[5] - sint[4])
      sa_parallel_match_neg(qsa, qint[4], qint[5], sa, sint[4], sint[5], offset,
                            depth, last_match_count + 1, mismatch_count,
                            results);
    /* wobble pair */
  ...
  /********************** MISMATCH CASES ***********************/
  /* MISMATCH can only start in specified position and can have only specific
   * length */
  if (seed_mismatch_flag) {
    ...
    if ((mismatch_count < seed_mismatch[0]) && (offset > seed_mismatch[1]) &&
      ...
      if (qint[1] - qint[0]) {
        if (sint[2] - sint[1])
          sa_parallel_match_neg(qsa, qint[0], qint[1], sa, sint[1], sint[2],
                                offset, depth, 0, mismatch_count + 1, results);
        if (sint[3] - sint[2])
          sa_parallel_match_neg(qsa, qint[0], qint[1], sa, sint[2], sint[3],
                                offset, depth, 0, mismatch_count + 1, results);
        if (sint[5] - sint[4])
          sa_parallel_match_neg(qsa, qint[0], qint[1], sa, sint[4], sint[5],
                                offset, depth, 0, mismatch_count + 1, results);
      }
      /* MISMATCH for c */
      if (qint[2] - qint[1]) {
        if (sint[1] - sint[0])
          sa_parallel_match_neg(qsa, qint[1], qint[2], sa, sint[0], sint[1],
                                offset, depth, 0, mismatch_count + 1, results);
        if (sint[3] - sint[2])
          sa_parallel_match_neg(qsa, qint[1], qint[2], sa, sint[2], sint[3],
                                offset, depth, 0, mismatch_count + 1, results);
        if (sint[5] - sint[4])
          sa_parallel_match_neg(qsa, qint[1], qint[2], sa, sint[4], sint[5],
                                offset, depth, 0, mismatch_count + 1, results);
      }
      ...
      if (qint[3] - qint[2]) {
        if (sint[2] - sint[1])
          sa_parallel_match_neg(qsa, qint[2], qint[3], sa, sint[1], sint[2],
                                offset, depth, 0, mismatch_count + 1, results);
        if (sint[5] - sint[4])
          sa_parallel_match_neg(qsa, qint[2], qint[3], sa, sint[4], sint[5],
                                offset, depth, 0, mismatch_count + 1, results);
        if (noGUseed)
          if (sint[1] - sint[0])
            sa_parallel_match_neg(qsa, qint[2], qint[3], sa, sint[0], sint[1],
                                  offset, depth, 0, mismatch_count + 1,
                                  results);
      }
      ...
      if (qint[5] - qint[4]) {
        if (sint[1] - sint[0])
          sa_parallel_match_neg(qsa, qint[4], qint[5], sa, sint[0], sint[1],
                                offset, depth, 0, mismatch_count + 1, results);
        if (sint[3] - sint[2])
          sa_parallel_match_neg(qsa, qint[4], qint[5], sa, sint[2], sint[3],
                                offset, depth, 0, mismatch_count + 1, results);
        if (noGUseed)
          if (sint[2] - sint[1])
            sa_parallel_match_neg(qsa, qint[4], qint[5], sa, sint[1], sint[2],
